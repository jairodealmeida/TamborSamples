...
public class Repository<T> {
    ...
    public void selectAll(List<Entity> entryes, Class<?> entity) {
        ...
        try {
            ...
            Log.i(context, "Time to query: " + (end - start) + " ms");
        }
    ...
    public TransferObject selectSingle(WhereStatement whereClause, OrderByStatement orderByClause) {
        ...
        try {
            ...
            Log.i(context, sql.createStatement().toString());
            c.moveToFirst();
            if (!c.isAfterLast()) {
                TransferObject bean = this.fill(c);
                return bean;
    ...
    private FieldTO getColumnIndex( Cursor c , Object type , String name ){
        int index = c.getColumnIndex(name);
        if (type instanceof String)
            return (new FieldTO(name, c.getString(index)));
        else if (type instanceof Integer)
            return (new FieldTO(name, c.getInt(index)));
        else if (type instanceof Long)
            return (new FieldTO(name, c.getLong(index)));
        else if (type instanceof BigDecimal)
            return (new FieldTO(name, c.getLong(index)));
        else if (type instanceof Double)
            return (new FieldTO(name, c.getDouble(index)));
        else if (type instanceof Float)
            return (new FieldTO(name, c.getFloat(index)));
        else if (type instanceof byte[])
            return (new FieldTO(name, c.getBlob(index)));
        else if (type instanceof Short)
            return (new FieldTO(name, c.getShort(index)));
        else {
            Log.e(context,"Name " + name + " Not exist in model", new Exception("Model attribute not found"));
            return null;
        }
    }
    ...
    private TransferObject fillTO(Cursor c) throws IllegalAccessException, InstantiationException {
        ...
        while (c.moveToNext()) {
            for (int col = 0; col < c.getColumnCount(); col++) {
                ...
                try {
                    fields.add(getColumnIndex(c,type,name));
                } catch (Exception e) {
                    Log.e(context, e.getMessage(), e);
                }
            }
    ...
    private FieldTO getFieldAt(Cursor c, String name, Field field, int columnIndex)
        ...
        try {
            ...
            return (getColumnIndex(c,type,name));
        } catch (Exception e) {
            Log.e(context, e.getMessage(), e);
        }
    ...
    private FieldTO getFieldAt(Cursor c, String name, int type, int columnIndex)
        ...
        try {
            ...
            Log.e(context, "fail to mapping " + name + " : " + e.getMessage(), e);
        }
        return null;
    }
