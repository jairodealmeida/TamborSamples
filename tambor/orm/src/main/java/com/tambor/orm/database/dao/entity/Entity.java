package com.tambor.orm.database.dao.entity;
...
public abstract class Entity implements Serializable{
	...
	public FieldTO getField(int fieldCol) {
		try {
			...
			reflectionField.setAccessible(true);
			String fieldName = reflectionField.getName();
			GPAField annoField = reflectionField.getAnnotation(GPAField.class);
			int fieldType = annoField.type();
			Object fieldValue = reflectionField.get(this);
			return new FieldTO(fieldName, fieldValue,fieldType);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
